...
if type brew &>/dev/null; then
  ...
  if [[ -e Brewfile ]]; then
    ...
    set +x
    for i in $(cat Brewfile $EXTRA_BREWFILE | grep cask | grep -v '^#' | cut -d' ' -f2 | tr -d '"' | grep -v -E '^(hammerspoon|google-chrome|docker-desktop|podman-desktop|visual-studio-code|1password|discord|lm-studio|lulu|signal|brave-browser|firefox)$'); do
      set -x
