...
if ! docker inspect $K3S_NODE_NAME >/dev/null ; then
  ...
  set +x
  docker logs $K3S_NODE_NAME 2>&1 | grep etc | grep -o -E '"initial-cluster":"[^"]*"' | sed 's,":",~,' | sed 's,.*~,,' | tr , "\n" | sed 's,=.*,,'
  echo "Enter a value for ETCD_NAME:"
  read ETCD_NAME
  set -x

  docker rm -f $K3S_NODE_NAME

  $K3S_CMD --server=$K8S_URL --etcd-arg="--name=$ETCD_NAME" --etcd-arg="--listen-peer-urls=https://0.0.0.0:2380" --etcd-arg="--listen-client-urls=https://0.0.0.0:2379" --etcd-arg="--initial-advertise-peer-urls=https://$IP_ADDRESS:2380" --etcd-arg="--advertise-client-urls=https://$IP_ADDRESS:2379" --datastore-endpoint="https://$IP_ADDRESS:2379"
fi
